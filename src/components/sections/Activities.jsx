  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm card-hover cursor-pointer group" onClick={() => onClick(item)}>
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <span className="absolute bottom-3 left-3 bg-[#E8C44A] text-[#1A2240] text-xs font-semibold px-2 py-0.5 rounded-full">{item.category}</span>
      </div>
      <div className="p-4">
...
export default function Activities({ addItem, removeItem, isSaved }) {
  ...
  return (
    <section className="section-container">
      <div className="mb-8">
        <h2 className="text-[#1B3A8C] text-3xl font-bold mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>Activities & Experiences</h2>
        <p className="text-[#7A82A8]">34 curated experiences — from cooking classes to volcano treks</p>
      </div>

      <div className="mb-6">
        <TownSearch placeholder="Search activities by city..." onSelect={setSearch} onChange={setSearch} />
      </div>
